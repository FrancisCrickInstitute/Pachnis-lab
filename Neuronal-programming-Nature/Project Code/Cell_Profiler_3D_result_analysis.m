...
%%project Cell Profiler pipeline.
...
for j = 1:size(data_table,2)   %iterate over all tracked cells
 ...
 %Write out csv file
 first_file = Filename_table{1,1};  %get the filename of the first file
 ...
 first_file_prompt = strcat(first_file_prefix, '_.xlsx');  %tag on an excel file extension so the computer saves it to excel
